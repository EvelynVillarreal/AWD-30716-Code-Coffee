...
This version documents what exists in the repository at ALCSystem v2.0.1.
...
The backend currently has **27 implemented route entries**:

- **1 non-API root route:** `GET /`
- **26 API routes:** routes under `/api`
...
| 10 | GET | `/api/students` | Yes | director | Lists students with optional filters. |
| 11 | POST | `/api/students` | Yes | director | Creates a student record. |
| 12 | PATCH | `/api/students/{studentId}` | Yes | director | Updates a student record. |
| 13 | DELETE | `/api/students/{studentId}` | Yes | director | Deactivates a student record. |
| 14 | GET | `/api/teachers` | Yes | director | Lists teacher accounts. |
| 15 | POST | `/api/teachers` | Yes | director | Creates a teacher account. |
| 16 | PATCH | `/api/teachers/{teacherId}` | Yes | director | Updates a teacher account. |
| 17 | DELETE | `/api/teachers/{teacherId}` | Yes | director | Deactivates a teacher account. |
| 18 | GET | `/api/class-plans` | Yes | teacher, director | Lists class planning records. |
| 19 | POST | `/api/class-plans` | Yes | teacher, director | Submits a class planning record. |
| 20 | GET | `/api/attendance-records` | Yes | teacher, director | Lists attendance records and teacher payroll summary. |
| 21 | POST | `/api/attendance-records` | Yes | teacher, director | Creates a manual attendance record. |
| 22 | GET | `/api/branch-finance-reports` | Yes | director | Lists branch finance reports. |
| 23 | POST | `/api/branch-finance-reports` | Yes | director | Creates a branch finance report. |
| 24 | GET | `/api/professional-events` | Yes | director | Lists professional B2 events. |
| 25 | POST | `/api/professional-events` | Yes | director | Creates a professional event. |
| 26 | POST | `/api/professional-events/{eventId}/assignments` | Yes | director | Assigns a B2 dancer to an event. |
| 27 | GET | `/api/dancer-settlements/{studentId}` | Yes | director | Calculates B2 dancer settlement summary. |
...
This document only includes URIs already implemented in ALCSystem v2.0.1.
...
ALCSystem currently includes a functional backend API with 27 implemented route entries and a frontend with 16 canonical route entries. The API supports public enrollment, authentication, role-based access, student and teacher management, attendance control, teacher payroll summaries, class planning, branch finance reports, and B2 professional event management.
