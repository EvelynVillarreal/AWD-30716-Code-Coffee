...
export default function ProductFormPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      alert('Product saved successfully! (Mock operation)');
  ...
  return (
    <ProtectedRoute adminOnly>
      <div className="container" style={{ padding: 'var(--space-12) var(--space-6)' }}>
        ...
        <h1 className="section-title">{isEditing ? 'Edit Product' : 'Add New Product'}</h1>
        <p className="section-subtitle">Fill in the product details below.</p>
        ...
        <div className="card" style={{ maxWidth: '800px' }}>
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="photoUrl">Product Image URL</label>
              ...
              <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 'var(--space-1)' }}>
                <strong>Image Upload Strategy:</strong> Since external links like Pinterest block embedding (hotlinking/CORS), 
                we recommend uploading your images to a public storage bucket (like Supabase Storage or Cloudinary) 
                and pasting the direct public URL here.
              </p>
            </div>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-4)', margin: 'var(--space-6) 0' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', cursor: 'pointer' }}>
                ...
                <span className="form-label" style={{ margin: 0 }}>Allows Customization</span>
              </label>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
              ...
              <button type="submit" disabled={isSaving} className="btn btn-primary">
                {isSaving ? <><span className="spinner" /> Saving...</> : 'Save Product'}
              </button>
            </div>
          </form>
        </div>
