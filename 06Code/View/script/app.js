...
async function apiRequest(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(payload.message || firstError(payload.errors) || "The request could not be completed.");
  }
...
function initEnrollmentPage() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      form.reset();
      fillBranchSelects();
      showMessage("enrollmentMessage", "Request submitted. The academy will review your information before activating the enrollment.");
    } catch (error) {
...
async function initDashboardPage() {
  ...
  setText("sessionRole", roleLabels[currentUser.role] || "Internal portal");
...
function renderStudentProfile() {
  ...
  container.innerHTML = `
    <article class="panel">
      ...
      <p>${escapeHtml(student.branch?.name || branchName(student.branch_id))} | ${escapeHtml(student.level)} | Scholarship ${Number(student.scholarship_percent || 0)}%</p>
    </article>
    <article class="panel">
...
function bindClassPlanForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setDefaultDates();
      setValue("planTeacher", currentUser.name);
      showMessage("planMessage", "Monthly plan submitted for review.");
    } catch (error) {
...
function bindAttendanceForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      form.reset();
      setDefaultDates();
      showMessage("attendanceMessage", `Attendance saved: ${payload.data.evidence_code}.`);
    } catch (error) {
...
function bindEventForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      dashboardData.events = eventsPayload.data || [];
      renderDashboard();
      showMessage("eventMessage", dancer ? "B2 event registered and assigned." : "Event registered. The dancer was not assigned because no active B2 match was found.");
    } catch (error) {
